	...
	if($noOfantecedent==2){
		...
		for($i=0;$i<$noOfantecedentRefVal[0];$i++){
			for($j=0;$j<$noOfantecedentRefVal[1];$j++){
				...
				//print_r($consequentRefVal);
                write_to_log(__FILE__,__LINE__,__FUNCTION__,'$consequentRefVal[$0]',$consequentRefVal[0]);
                write_to_log(__FILE__,__LINE__,__FUNCTION__,'$consequentRefVal[$1]',$consequentRefVal[1]);
                write_to_log(__FILE__,__LINE__,__FUNCTION__,'$consequentRefVal[$2]',$consequentRefVal[2]);
                write_to_log(__FILE__,__LINE__,__FUNCTION__,'$value[$0]',$value[0]);
                write_to_log(__FILE__,__LINE__,__FUNCTION__,'$value[$1]',$value[1]);
                write_to_log(__FILE__,__LINE__,__FUNCTION__,'$value[$2]',$value[2]);
                ...
                for($l=0;$l<$noOfConsequentRefVal-1;$l++){
                    ...
                    If (($consequentRefVal[$l] > $y) && ($y > $consequentRefVal[$l + 1]) ){
                        write_to_log(__FILE__,__LINE__,__FUNCTION__,'$l',$l);
...
function read_sensor_data($consequent_id){
	//write_to_log(__FILE__,__LINE__,__FUNCTION__,'$consequent_id',$consequent_id);
	//$data=getSensorData($consequent_id);
...
// 		print_r($element_header);
		write_to_log(__FILE__,__LINE__,__FUNCTION__,'$element_header',$element_header);
...
// 	print_r($data);
	...
	foreach($data as $x => $x_value) {
		...
		for($i=0;$i<$_POST['number_of_child'];$i++){
			if($_POST['number_of_child']!=$x){
...
function read_sensor_data_realtime(){
    //write_to_log(__FILE__,__LINE__,__FUNCTION__,'$consequent_id',$consequent_id);
    $data=getSensorData();
...
function perform_calculate_sensor_data_realtime(){
    $sensor_data=read_sensor_data_realtime();

    foreach($sensor_data as $x => $x_value) {
        write_to_log(__FILE__,__LINE__,__FUNCTION__,'$x',$x);
        write_to_log(__FILE__,__LINE__,__FUNCTION__,'$x_value',$x_value);

    }
    write_to_log(__FILE__,__LINE__,__FUNCTION__,'$sensor_data',$sensor_data);
    $_POST['antecedent_input_1']=$sensor_data['temp'][0];
    $_POST['antecedent_input_2']=$sensor_data['soil_mois'][0];
    write_to_log(__FILE__,__LINE__,__FUNCTION__,'$_POST[\'antecedent_input_1\']',$_POST['antecedent_input_1']);
...
// 				print_r($child_data_ass);
		...
		write_to_log(__FILE__,__LINE__,__FUNCTION__,'count($child_data_row)',count($child_data_row));
...
//   			$_SESSION[$_GET['node']]["sensor_graph_label_".$k]=data_normalization($_SESSION[$_GET['node']]["sensor_graph_label_".$k]);
	...
	for($k=1;$k<=$_POST['number_of_child'];$k++){
		...
		$dataTObeNormalize2[]=data_normalization($_SESSION[$_GET['node']]["sensor_graph_label_".$k]);
	}
	m_data_normalization($dataTObeNormalize);
...
function m_max($data){
	...
	write_to_log(__FILE__,__LINE__,__FUNCTION__,'$$max',$max);
	return $max;
}
function m_min($data){
	$min=99999;
	...
	write_to_log(__FILE__,__LINE__,__FUNCTION__,'$min',$min);
	return $min;
}
function m_data_normalization($dataNS){
	 	write_to_log(__FILE__,__LINE__,__FUNCTION__,'$dataNS',$dataNS);
	 	$max=m_max($dataNS);
	 	write_to_log(__FILE__,__LINE__,__FUNCTION__,'m_max($dataNS)',m_max($dataNS));
	 	$min=m_min($dataNS);
	 	write_to_log(__FILE__,__LINE__,__FUNCTION__,'m_smin($dataNS)',m_min($dataNS));
	for($i=0;$i<count($dataNS);$i++){
		for($j=0;$j<count($dataNS[0]);$j++)
		$data[$i][$j]=($dataNS[$i][$j]-$min)/($max-$min);
		// 		write_to_log(__FILE__,__LINE__,__FUNCTION__,'$dataNS['.$i.']',$dataNS[$i]);
		// 		$dataNS[$i]=$dataNS[$i]*100;
		// 		write_to_log(__FILE__,__LINE__,__FUNCTION__,'$dataNS['.$i.']',$dataNS[$i]);

	}
	write_to_log(__FILE__,__LINE__,__FUNCTION__,'$dataNS',$data);
	return $data;
...
if($_POST)
{
	...
	if(isset($_GET['action_type']) && !empty($_GET['action_type'])){
		...
		if ($_GET['action_type']=="calculate_sensor_data_rt"){
            write_to_log(__FILE__,__LINE__,__FUNCTION__,' $_GET[\'action_type\']', $_GET['action_type']);
            $result_status=perform_calculate_sensor_data_realtime();
            write_to_log(__FILE__,__LINE__,__FUNCTION__,' $result_status', $result_status);
            if($result_status=='done'){
               ...
               /* $data['horizontalAxisLabel']== array("X22", "X23");
                ...
                $data['returntag_status'] = '.status';
                $html_status='Performed Computation for realtime sensor data';
